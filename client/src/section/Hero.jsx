...
const Sidebar = ({ isOpen, onClose }) => {
	...
	return (
		<>
			...
			<div
				className={`fixed top-0 left-0 h-full w-64 bg-white transform transition-transform duration-300 ease-in-out z-50 flex flex-col shadow-2xl ${
					...
					<nav className="p-4">
						<ul className="space-y-4">
							{menuItems.map((item, index) => (
								<li key={index}>
									<button className="flex items-center gap-4 w-full px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">
										<item.icon size={20} />
						...
						<div className="flex items-center justify-center pb-24">
							<p className="text-gray-400 text-xs">Version 89799.00</p>
						</div>
					</nav>
				</div>
			</div>
			...
			<div className="container mx-auto px-5 py-10">
				...
				<div className="mt-8 flex items-center justify-between">
					<div className="flex items-center gap-2">
						<img src={profile} alt="Logo" className="h-14" />
						<div>
